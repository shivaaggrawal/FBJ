...
def create_bounty():
    return client.post("/api/bounties", json={"contract_bounty_id": "0x" + "ab" * 32, "repository": "owner/demo-repository",
        "issue_url": "https://github.com/owner/demo-repository/issues/1", "reward_token": "0x" + "12" * 20,
...
def test_valid_supported_webhook_is_accepted():
    ...
    payload = b'{"action":"opened","number":1,"repository":{"full_name":"owner/demo-repository"},"pull_request":{"head":{"sha":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},"title":"Test","user":{"login":"developer"}}}'
    signature = hmac.new(b"replace-me", payload, hashlib.sha256).hexdigest()
...
def test_duplicate_delivery_is_idempotent():
    payload = b'{"action":"opened","number":2,"repository":{"full_name":"owner/demo-repository"},"pull_request":{"head":{"sha":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},"title":"Test","user":{"login":"developer"}}}'
    signature = hmac.new(b"replace-me", payload, hashlib.sha256).hexdigest()
